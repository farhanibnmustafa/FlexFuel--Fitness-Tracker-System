...
const editPerformanceBtn = document.getElementById('editPerformanceBtn');
...
const performanceEmptyEl = document.getElementById('performanceEmpty');
const performanceSummaryEl = document.getElementById('performanceSummary');
const performanceTitleEl = document.getElementById('performanceWorkoutTitle');
const performanceMetaEl = document.getElementById('performanceWorkoutMeta');
const performanceStatusEl = document.getElementById('performanceStatus');
const performanceWatchedEl = document.getElementById('performanceWatched');
const performancePercentEl = document.getElementById('performancePercent');
const performanceTargetEl = document.getElementById('performanceTarget');
const performanceExerciseListEl = document.getElementById('performanceExerciseList');
...
const performanceEditModal = document.getElementById('performanceEditModal');
const performanceEditClose = document.getElementById('performanceEditClose');
const performanceEditForm = document.getElementById('performanceEditForm');
const performanceEditList = document.getElementById('performanceEditList');
...
const calendarDetailSummary = document.getElementById('calendarDetailSummary');
...
let workoutPlanCache = { start: null, end: null, workoutsByDate: {} };
...
const fetchHabitLogsFromServer = async () => {
  ...
  try {
    ...
    console.warn('Failed to load habit logs from server', err);
    return [];
  }
};

const persistHabitLogsToServer = async (entries) => {
  if (!currentUserId || !Array.isArray(entries) || !entries.length) return;
  try {
    ...
    console.warn('Failed to save habit logs', err);
  }
};

const syncHabitLogsFromServer = async () => {
  const serverEntries = await fetchHabitLogsFromServer();
  if (!serverEntries.length) return;
  const localHabits = readStorage(storageKey('habits'), { sleep: {}, nosugar: {}, water: {} });
  if (!localHabits.water) localHabits.water = {};

  serverEntries.forEach((entry) => {
    const date = entry?.habit_date;
    const key = entry?.habit_key;
    if (!date || !key) return;
    if (!localHabits[key]) localHabits[key] = {};
    if (key === 'nosugar') {
      localHabits[key][date] = Boolean(entry?.status);
    } else {
      const value = Number(entry?.value);
      if (Number.isFinite(value)) {
        localHabits[key][date] = value;
      }
    }
  });

  writeStorage(storageKey('habits'), localHabits);

  const localEntries = [];
  Object.entries(localHabits || {}).forEach(([habitKey, dates]) => {
  ...
  if (localEntries.length) {
    await persistHabitLogsToServer(localEntries);
  }
};

const getWorkoutUserKey = () => (currentUserId ? `user:${currentUserId}` : 'anonymous');
...
const normalizeKey = (text) => String(text || '').toLowerCase().replace(/\s+/g, '').replace(/-/g, '');

const loadWorkoutDayCache = () => {
  const key = `${WORKOUT_DAY_CACHE_KEY}.${getWorkoutUserKey()}`;
  try {
    const raw = localStorage.getItem(key);
    if (!raw) return null;
    const parsed = JSON.parse(raw);
    if (!parsed || typeof parsed !== 'object') return null;
    if (!parsed.day || !parsed.dateIso) return null;
    return parsed;
  } catch (err) {
    console.warn('Failed to read workout day cache', err);
    return null;
  }
};
...
const loadExerciseCompletionState = () => {
  try {
    const raw = localStorage.getItem(WORKOUT_EXERCISE_COMPLETION_KEY);
    if (!raw) return {};
    const parsed = JSON.parse(raw);
...
const normalizeExercisePerformance = (exercise) => {
  const metrics = exercise && typeof exercise.metrics === 'object' ? exercise.metrics : {};
  const targets = parsePrescriptionTargets(exercise?.prescription);
  const targetSets = coalesceNumber(exercise?.targetSets, metrics.targetSets, targets.sets);
  const targetReps = coalesceNumber(exercise?.targetReps, metrics.targetReps, targets.reps);
  const targetDurationSec = coalesceNumber(exercise?.targetDurationSec, metrics.targetDurationSec, targets.durationSec);
  const performedSets = coalesceNumber(exercise?.performedSets, metrics.performedSets, targetSets);
  const performedReps = coalesceNumber(exercise?.performedReps, metrics.performedReps, targetReps);
  const performedWeight = coalesceNumber(exercise?.performedWeight, metrics.performedWeight);
  let watchedSeconds = coalesceNumber(exercise?.watchedSeconds, metrics.watchedSeconds);
  let videoDurationSec = coalesceNumber(exercise?.videoDurationSec, metrics.videoDurationSec);
  let percentWatched = coalesceNumber(
    exercise?.percentWatched,
    metrics.percentWatched,
    videoDurationSec && Number.isFinite(watchedSeconds) ? (watchedSeconds / videoDurationSec) * 100 : null
  );
  let completedFlag = false;
  if (typeof exercise?.completed === 'boolean') {
    completedFlag = exercise.completed;
  } else if (typeof exercise?.videoCompleted === 'boolean') {
    completedFlag = exercise.videoCompleted;
  } else if (typeof metrics?.completed === 'boolean') {
    completedFlag = metrics.completed;
  } else if (Number.isFinite(percentWatched)) {
    completedFlag = percentWatched >= 90;
  }

  if (completedFlag && !Number.isFinite(percentWatched)) {
    percentWatched = 100;
  }
  if (completedFlag && !Number.isFinite(watchedSeconds)) {
    const durationFallback = coalesceNumber(
      exercise?.targetDurationSec,
      metrics.targetDurationSec,
      targetDurationSec
    );
    if (Number.isFinite(durationFallback)) {
      watchedSeconds = durationFallback;
      if (!Number.isFinite(videoDurationSec)) {
        videoDurationSec = durationFallback;
      }
    }
  }
  ...
  return {
    ...
    watchedSeconds,
    videoDurationSec,
    percentWatched: Number.isFinite(percentWatched) ? Math.min(100, Math.round(percentWatched)) : null,
    completed: completedFlag
  };
};

const formatTargetSummary = (metrics) => {
  if (!metrics) return '--';
  if (metrics.targetSets && metrics.targetReps) {
    return `${metrics.targetSets} sets × ${metrics.targetReps} reps`;
  }
  if (metrics.targetDurationSec) {
    return `Time: ${formatDuration(metrics.targetDurationSec)}`;
  }
  if (metrics.targetSets) {
    return `${metrics.targetSets} sets`;
  }
  if (metrics.targetReps) {
    return `${metrics.targetReps} reps`;
  }
  return '--';
};

const formatPerformanceLabel = (percent) => {
  if (!Number.isFinite(percent)) return 'Getting started';
...
const computeEffortRatio = (metrics) => {
  if (!metrics) return null;
  const ratios = [];
  if (Number.isFinite(metrics.targetSets) && metrics.targetSets > 0) {
    const performed = Number(metrics.performedSets || 0);
    ratios.push(clampRatio(performed / metrics.targetSets));
  }
  if (Number.isFinite(metrics.targetReps) && metrics.targetReps > 0) {
    const performed = Number(metrics.performedReps || 0);
    ratios.push(clampRatio(performed / metrics.targetReps));
  }
  if (Number.isFinite(metrics.targetDurationSec) && metrics.targetDurationSec > 0) {
    const performed = Number(metrics.watchedSeconds || 0);
    ratios.push(clampRatio(performed / metrics.targetDurationSec));
  }
  if (!ratios.length) return null;
  const total = ratios.reduce((sum, value) => sum + (value ?? 0), 0);
  return total / ratios.length;
};

const computeWatchRatio = (metrics) => {
  if (!metrics) return null;
  if (Number.isFinite(metrics.percentWatched)) {
    return clampRatio(metrics.percentWatched / 100);
  }
  if (Number.isFinite(metrics.watchedSeconds) && Number.isFinite(metrics.videoDurationSec) && metrics.videoDurationSec > 0) {
    return clampRatio(metrics.watchedSeconds / metrics.videoDurationSec);
  }
  return null;
};

const computePerformanceScore = (metrics) => {
  const watchRatio = computeWatchRatio(metrics);
  const effortRatio = computeEffortRatio(metrics);
  if (Number.isFinite(watchRatio) && Number.isFinite(effortRatio)) {
    return Math.round((watchRatio * 0.5 + effortRatio * 0.5) * 100);
  }
  if (Number.isFinite(effortRatio)) {
    return Math.round(effortRatio * 100);
  }
  if (Number.isFinite(watchRatio)) {
    return Math.round(watchRatio * 100);
  }
  return null;
};
...
function renderBodyComposition() {
  ...
  const startingWeight = Number(currentProfile?.starting_weight_kg) || null;
  ...
  if (series.length) {
    if (startingWeight) {
      const firstWeight = Number(series[0].weight);
      if (!Number.isFinite(firstWeight) || firstWeight !== startingWeight) {
        const startDate = shiftISODate(series[0].date, -1) || series[0].date || todayIso;
        series = [{ date: startDate, weight: startingWeight }, ...series];
      }
    }
  } else if (startingWeight) {
    series = [{ date: todayIso, weight: startingWeight }];
    if (profileWeight && profileWeight !== startingWeight) {
      series.push({ date: todayIso, weight: profileWeight });
    }
  ...
  const startWeight = startingWeight || series[0]?.weight || profileWeight || null;
...
async function loadLatestWorkoutSession() {
  try {
    const todayIso = toISODate(new Date());
    const cached = loadWorkoutDayCache();
    if (cached?.day && cached.dateIso === todayIso) {
      const completionState = loadExerciseCompletionState();
      const userKey = getWorkoutUserKey();
      const fallbackKey = userKey === 'anonymous' ? null : 'anonymous';
      const dayBucket = {
        ...(completionState?.[userKey]?.[todayIso] || {}),
        ...(fallbackKey ? completionState?.[fallbackKey]?.[todayIso] || {} : {})
      };
      const workouts = Array.isArray(cached.day.exercises) ? cached.day.exercises : [];
      const normalizedWorkouts = workouts.map((exercise, index) => {
        if (!exercise || typeof exercise !== 'object') return exercise;
        const exerciseKey = exercise.id
          ? String(exercise.id)
          : normalizeKey(
              `${String(exercise.name || '').trim()}::${String(exercise.prescription || '').trim()}::${index}`
            );
        const completed = Boolean(dayBucket?.[exerciseKey]) || Boolean(exercise.completed);
        return { ...exercise, completed };
      });
      const hasCompleted = normalizedWorkouts.some((exercise) => exercise && exercise.completed);
      latestWorkoutSession = {
        ...
        level_code: cached.day.level || null,
        day_index: cached.day.dayNumber || cached.day.day || null,
        goal_slug: cached.day.goalSlug || cached.day.goal || null
      };
      return;
    }

    const res = await fetch('/api/workouts/latest');
    if (!res.ok) throw new Error('Failed to load latest workout');
    const data = await res.json();
    const session = data?.session || null;
    if (session && !Array.isArray(session.workouts)) {
      session.workouts = [];
    }
    latestWorkoutSession = session;
  } catch (err) {
    console.error('Failed to load latest workout session', err);
    latestWorkoutSession = null;
  }
}

async function loadPerformanceSummary() {
  try {
    const res = await fetch('/api/workouts/performance-summary');
...
function renderPerformanceMilestones() {
  const session = latestWorkoutSession;
  const workouts = Array.isArray(session?.workouts) ? session.workouts : [];
  const hasSession = Boolean(session && workouts.length);
  const sessionCompleted = Boolean(session?.completed_at || session?.status === 'completed');

  if (!performanceSummaryEl || !performanceExerciseListEl) return;

  if (!hasSession) {
    performanceSummaryEl.hidden = true;
    if (performanceEmptyEl) performanceEmptyEl.hidden = false;
    if (editPerformanceBtn) editPerformanceBtn.disabled = true;
    return;
  }

  if (performanceEmptyEl) performanceEmptyEl.hidden = true;
  performanceSummaryEl.hidden = false;
  if (editPerformanceBtn) editPerformanceBtn.disabled = false;

  const goalLabel = session.goal_slug
    ? session.goal_slug.replace(/-/g, ' ').replace(/\b\w/g, (char) => char.toUpperCase())
    : 'Workout Session';
  const dayLabel = session.day_index ? `Day ${session.day_index}` : '';
  if (performanceTitleEl) {
    performanceTitleEl.textContent = [goalLabel, dayLabel].filter(Boolean).join(' • ');
  }

  const completedAt = session.completed_at
    ? new Date(session.completed_at)
    : session.scheduled_date
    ? new Date(`${session.scheduled_date}T00:00:00`)
    : null;
  if (performanceMetaEl) {
    performanceMetaEl.textContent = `Last completed: ${completedAt ? formatDateLabel(completedAt) : 'TBD'}`;
  }
  ...
  const rows = workouts
    .map((exercise) => {
      const metrics = normalizeExercisePerformance(exercise);
      const name = exercise?.name || exercise?.exercise || 'Workout';
      const targetLabel = formatTargetSummary(metrics);
      const categoryLabel = Array.isArray(exercise?.categories) && exercise.categories.length
        ? exercise.categories
            .map((cat) => String(cat || '').trim())
            .filter(Boolean)
            .map((cat) => cat.replace(/-/g, ' ').replace(/\b\w/g, (char) => char.toUpperCase()))
            .slice(0, 2)
            .join(', ')
        : null;
      const goalLabel = session?.goal_slug
        ? session.goal_slug.replace(/-/g, ' ').replace(/\b\w/g, (char) => char.toUpperCase())
        : null;
      const targetPieces = [categoryLabel, goalLabel].filter(Boolean);
      const targetContext = targetPieces.length ? ` • ${targetPieces.join(' • ')}` : '';
      const setsLabel = Number.isFinite(metrics.performedSets) ? `${metrics.performedSets}` : '--';
      const repsLabel = Number.isFinite(metrics.performedReps) ? `${metrics.performedReps}` : '--';
      const weightLabel = Number.isFinite(metrics.performedWeight) ? `${formatNumber(metrics.performedWeight, 'kg')}` : '--';

      const resolvedCompleted = metrics.completed || Boolean(exercise?.completed);
      const fallbackDuration = Number.isFinite(metrics.targetDurationSec) ? metrics.targetDurationSec : null;
      const displayWatchedSeconds =
        Number.isFinite(metrics.watchedSeconds) && metrics.watchedSeconds > 0
          ? metrics.watchedSeconds
          : resolvedCompleted && fallbackDuration
          ? fallbackDuration
          : null;
      const displayPercent =
        Number.isFinite(metrics.percentWatched) && metrics.percentWatched > 0
          ? metrics.percentWatched
          : resolvedCompleted
          ? 100
          : null;
      const totalDurationLabel = Number.isFinite(metrics.videoDurationSec) && metrics.videoDurationSec > 0
        ? formatDuration(metrics.videoDurationSec)
        : null;
      const watchedLabel = displayWatchedSeconds ? formatDuration(displayWatchedSeconds) : '--';
      const watchedWithTotal = totalDurationLabel ? `${watchedLabel} / ${totalDurationLabel}` : watchedLabel;
      const hasLoadData =
        Number.isFinite(metrics.performedSets) ||
        Number.isFinite(metrics.performedReps) ||
        Number.isFinite(metrics.performedWeight);
      const performanceScore = computePerformanceScore(metrics);
      let exercisePerformanceLabel = formatPerformanceLabel(
        Number.isFinite(performanceScore) ? performanceScore : displayPercent
      );
      if (hasLoadData && exercisePerformanceLabel !== 'Excellent' && Number.isFinite(performanceScore)) {
        if (exercisePerformanceLabel === 'Getting started') exercisePerformanceLabel = 'Good';
        else if (exercisePerformanceLabel === 'Needs Work') exercisePerformanceLabel = 'Fair';
      }
      const percentLabel = Number.isFinite(displayPercent) && displayPercent > 0 ? `${displayPercent}%` : '--';

      totalWatched += Number.isFinite(displayWatchedSeconds) ? displayWatchedSeconds : 0;
      totalDuration += Number.isFinite(metrics.videoDurationSec)
        ? metrics.videoDurationSec
        : Number.isFinite(fallbackDuration)
        ? fallbackDuration
        : 0;
      totalTargetSets += metrics.targetSets || 0;
      totalTargetReps += metrics.targetReps || 0;
      totalTargetDuration += metrics.targetDurationSec || 0;
      if (resolvedCompleted) completedCount += 1;
      if (Number.isFinite(displayPercent)) {
        percentSum += displayPercent;
        percentCount += 1;
      }
      if (Number.isFinite(performanceScore)) {
        scoreSum += performanceScore;
        scoreCount += 1;
      }
      const hasWatchActivity =
        (Number.isFinite(displayWatchedSeconds) && displayWatchedSeconds > 0) ||
        (Number.isFinite(displayPercent) && displayPercent > 0);
      const statusClass = resolvedCompleted ? '' : 'is-incomplete';
      const statusLabel = resolvedCompleted ? 'Completed' : 'Not Completed';
      const shouldShow = hasWatchActivity;

      return {
        html: `
        <div class="performance-exercise">
          <div class="performance-exercise-header">
            <div>
              <div class="performance-exercise-title">${name}</div>
              <div class="performance-exercise-meta">Target: ${targetLabel}${targetContext}</div>
            </div>
            <span class="performance-chip ${statusClass}">${statusLabel}</span>
          </div>
          <div class="performance-exercise-stats">
            <span>Watched: ${watchedWithTotal}</span>
            <span>${percentLabel} watched</span>
            <span>Sets: ${setsLabel}</span>
            <span>Reps: ${repsLabel}</span>
            <span>Weight: ${weightLabel}</span>
            <span>Performance: ${exercisePerformanceLabel}</span>
          </div>
        </div>
      `,
  ...
  performanceExerciseListEl.innerHTML =
    rows || '<p class="empty-state">Watch an exercise or log your sets to see it here.</p>';
  ...
  const overallPercent =
    totalDuration > 0
      ...
      : null;
  const overallScore = scoreCount ? Math.round(scoreSum / scoreCount) : overallPercent;
  const overallComplete = completedCount > 0 ? completedCount === workouts.length : false;

  const hasWatchData = totalDuration > 0 || percentCount > 0 || totalWatched > 0;
  const summaryWatched = Number(performanceSummary?.totalWatchedSeconds);
  const summaryTarget = Number(performanceSummary?.totalTargetSeconds);
  const summaryPercent = Number(performanceSummary?.percentWatched);
  const summaryLabel = performanceSummary?.performanceLabel;

  if (performanceWatchedEl) {
    performanceWatchedEl.textContent = Number.isFinite(summaryWatched)
      ? formatDuration(summaryWatched)
      : hasWatchData
      ? formatDuration(totalWatched)
      : '--';
  }
  if (performancePercentEl) {
    performancePercentEl.textContent = Number.isFinite(overallScore)
      ? `${overallScore}%`
  ...
  if (performanceTargetEl) {
    const resolvedLabel = formatPerformanceLabel(Number.isFinite(overallScore) ? overallScore : summaryPercent);
    performanceTargetEl.textContent = resolvedLabel;
  }
  if (performanceStatusEl) {
    performanceStatusEl.textContent = overallComplete ? 'Completed' : 'Not Completed';
    performanceStatusEl.classList.toggle('is-incomplete', !overallComplete);
  }
}
...
function ensureWorkoutPlanCache(viewDate) {
  if (!window.FlexPlan || !(viewDate instanceof Date)) return;
  const viewIso = toISODate(new Date(viewDate.getFullYear(), viewDate.getMonth(), 15));
  if (workoutPlanCache.start && workoutPlanCache.end && viewIso >= workoutPlanCache.start && viewIso <= workoutPlanCache.end) {
    return;
  }
  ...
  try {
    ...
    console.warn('Failed to build workout plan cache', err);
  }
  const endDate = new Date(startDate);
  endDate.setDate(endDate.getDate() + days - 1);
  workoutPlanCache = {
    ...
    workoutsByDate: plan?.workoutsByDate || {}
  };
}

function getExerciseCompletionBucket(iso) {
  const completionState = loadExerciseCompletionState();
  const userKey = getWorkoutUserKey();
  const fallbackKey = userKey === 'anonymous' ? null : 'anonymous';
  return {
    ...(completionState?.[userKey]?.[iso] || {}),
    ...(fallbackKey ? completionState?.[fallbackKey]?.[iso] || {} : {})
  };
}

function getWorkoutPlanForDate(iso) {
  if (!iso) return null;
  const dateObj = parseISODate(iso);
  if (dateObj) {
    ensureWorkoutPlanCache(dateObj);
  }
  return workoutPlanCache.workoutsByDate?.[iso] || null;
}

function makeExerciseKey(exercise, index) {
  ...
  return normalizeKey(`${name}::${prescription}::${index}`);
}

function getWorkoutProgressForDate(iso) {
  const plan = getWorkoutPlanForDate(iso);
  const exercises = Array.isArray(plan?.exercises) ? plan.exercises : [];
  if (!exercises.length) {
    return { completed: 0, total: 0, exercises };
  }
  const bucket = getExerciseCompletionBucket(iso);
  let completed = 0;
  exercises.forEach((exercise, index) => {
    const key = makeExerciseKey(exercise, index);
    const isComplete = Boolean(exercise.completed) || (key && bucket?.[key]);
    if (isComplete) completed += 1;
  });
  return { completed, total: exercises.length, exercises };
}
...
function setupCalendarControls() {
  ...
  if (consistencyCalendarEl) {
    consistencyCalendarEl.addEventListener('click', (event) => {
      const cell = event.target.closest('.calendar-day');
      if (!cell || cell.classList.contains('is-empty')) return;
      const iso = cell.dataset.iso;
      if (iso) openCalendarDetail(iso);
    });
  }
  ...
  if (calendarDetailModal) {
    calendarDetailModal.addEventListener('click', (event) => {
      if (event.target === calendarDetailModal) closeCalendarDetail();
    });
  }
}
...
function renderCalendar() {
  ...
  ensureWorkoutPlanCache(viewDate);
  ...
  for (let day = 1; day <= daysInMonth; day += 1) {
    ...
    const progressLabel = `${status.score}/3`;
    cells.push(`
      <div class=\"calendar-day ${status.className}\" data-iso=\"${iso}\">
        ${day}
        <span class=\"day-score\">${progressLabel}</span>
        <div class="calendar-tooltip">
          <div class="calendar-tooltip-title">Daily Plans</div>
          <div class="calendar-tooltip-lines">
            ${tooltip.map((label) => `<span class="calendar-tooltip-line">${label}</span>`).join('')}
          </div>
        </div>
      </div>
    `);
...
function openCalendarDetail(iso) {
  ...
  const dateObj = parseISODate(iso);
  const label = dateObj ? formatDateLabel(dateObj) : iso;
  if (calendarDetailDate) {
    calendarDetailDate.textContent = label ? `Daily Plans • ${label}` : 'Daily Plans';
  }

  const status = getDaySystemStatus(iso);
  const workoutProgress = getWorkoutProgressForDate(iso);
  const completionMap = getMealCompletionMap();
  const completedMeals = Array.isArray(completionMap?.[iso]) ? completionMap[iso].length : 0;
  const dateObjForMeals = parseISODate(iso);
  if (dateObjForMeals) {
    ensureMealPlanCache(dateObjForMeals);
  }
  const totalMeals = mealPlanCache.mealsByDate?.[iso]?.meals?.length || 0;
  const mealProgressLabel = totalMeals > 0 ? `${completedMeals}/${totalMeals} meals` : 'No meals scheduled';
  const waterAmount = getWaterAmountForDate(iso);
  const sleepHours = Number(habitState.sleep?.[iso]);
  const sugarEntry = habitState.nosugar?.[iso];
  const waterLabel = waterTargetToday ? `${waterAmount}/${waterTargetToday} L` : `${waterAmount} L`;
  const sleepLabel = Number.isFinite(sleepHours) ? `${sleepHours} hrs` : 'Not logged';
  const sugarLabel = sugarEntry === true ? 'No sugar' : sugarEntry === false ? 'Had sugar' : 'Not logged';

  if (calendarDetailSummary) {
    calendarDetailSummary.textContent = `Plans hit: ${status.score}/3 (Workout • Meals • Habits).`;
  }

  if (calendarDetailList) {
    const rows = [];
    rows.push(`
      <li class="calendar-detail-item ${status.workoutDone ? 'is-complete' : ''}">
        <div class="calendar-detail-name">Workout plan</div>
        <div class="calendar-detail-meta">${
          workoutProgress.total > 0
            ? `${workoutProgress.completed}/${workoutProgress.total} completed`
            : 'No workout scheduled'
        }</div>
        <div class="calendar-detail-status">${status.workoutDone ? 'Completed' : 'Planned'}</div>
      </li>
    `);
    rows.push(`
      <li class="calendar-detail-item ${status.mealDone ? 'is-complete' : ''}">
        <div class="calendar-detail-name">Meal plan</div>
        <div class="calendar-detail-meta">${mealProgressLabel}</div>
        <div class="calendar-detail-status">${status.mealDone ? 'Completed' : 'Planned'}</div>
      </li>
    `);
    rows.push(`
      <li class="calendar-detail-item ${status.habitsDone ? 'is-complete' : ''}">
        <div class="calendar-detail-name">Hydration + Sleep + Sugar</div>
        <div class="calendar-detail-meta">${waterLabel} • ${sleepLabel} • ${sugarLabel}</div>
        <div class="calendar-detail-status">${status.habitsDone ? 'Completed' : 'Planned'}</div>
      </li>
    `);

    if (workoutProgress.total > 0) {
      rows.push(`
        <li class="calendar-detail-item">
          <div class="calendar-detail-name">Workout details</div>
          <div class="calendar-detail-meta">Tap an exercise below to see the plan.</div>
          <div class="calendar-detail-status">${workoutProgress.total} items</div>
        </li>
      `);
      const bucket = getExerciseCompletionBucket(iso);
      workoutProgress.exercises.forEach((exercise, index) => {
        const name = exercise?.name || exercise?.exercise || 'Exercise';
        const meta = exercise?.prescription || exercise?.detail || '';
        const key = makeExerciseKey(exercise, index);
        const completed = Boolean(exercise?.completed) || (key && bucket?.[key]);
        rows.push(`
          <li class="calendar-detail-item ${completed ? 'is-complete' : ''}">
            <div class="calendar-detail-name">${name}</div>
            <div class="calendar-detail-meta">${meta || 'No prescription'}</div>
            <div class="calendar-detail-status">${completed ? 'Completed' : 'Planned'}</div>
          </li>
        `);
      });
    }

    calendarDetailList.innerHTML = rows.join('');
  }

  calendarDetailModal.classList.add('show');
}
  ...
  if (checkinForm) {
    checkinForm.addEventListener('submit', async (event) => {
      ...
      try {
        ...
        await persistHabitLogsToServer([
          { habit_date: todayIso, habit_key: 'sleep', value: sleepHours },
          { habit_date: todayIso, habit_key: 'nosugar', status: sugarChoice === 'no' },
          { habit_date: todayIso, habit_key: 'water', value: waterAmount }
        ]);
...
function buildPerformanceEditRow(exercise, index) {
  const metrics = normalizeExercisePerformance(exercise);
  const row = document.createElement('div');
  row.className = 'performance-edit-row';
  const targetLabel = formatTargetSummary(metrics);
  const name = exercise?.name || exercise?.exercise || 'Workout';
  row.innerHTML = `
    <h4>${name}</h4>
    <p class="performance-edit-hint">Target: ${targetLabel}</p>
    <div class="performance-edit-fields">
      <div>
        <label for="perf-sets-${index}">Sets</label>
        <input id="perf-sets-${index}" type="number" min="0" step="1" value="${metrics.performedSets ?? ''}" data-field="performedSets" data-index="${index}" />
      </div>
      <div>
        <label for="perf-reps-${index}">Reps</label>
        <input id="perf-reps-${index}" type="number" min="0" step="1" value="${metrics.performedReps ?? ''}" data-field="performedReps" data-index="${index}" />
      </div>
      <div>
        <label for="perf-weight-${index}">Weight (kg)</label>
        <input id="perf-weight-${index}" type="number" min="0" step="0.1" value="${metrics.performedWeight ?? ''}" data-field="performedWeight" data-index="${index}" />
      </div>
    </div>
  `;
  return row;
...
function setupPerformanceEditModal() {
  if (editPerformanceBtn) {
    editPerformanceBtn.addEventListener('click', () => {
      if (!performanceEditModal || !performanceEditList) return;
      if (!latestWorkoutSession?.workouts?.length) return;
      performanceEditList.innerHTML = '';
      latestWorkoutSession.workouts.forEach((exercise, index) => {
        performanceEditList.appendChild(buildPerformanceEditRow(exercise, index));
      });
      performanceEditModal.classList.add('show');
    });
  }

  if (performanceEditClose) {
    performanceEditClose.addEventListener('click', () => {
      performanceEditModal?.classList.remove('show');
    });
  }

  if (performanceEditModal) {
    performanceEditModal.addEventListener('click', (event) => {
      if (event.target === performanceEditModal) {
        performanceEditModal.classList.remove('show');
      }
    });
  }

  if (performanceEditForm) {
    performanceEditForm.addEventListener('submit', async (event) => {
      event.preventDefault();
      if (!latestWorkoutSession?.id || !Array.isArray(latestWorkoutSession.workouts)) return;
      const submitBtn = performanceEditForm.querySelector('.submit-btn');
      if (submitBtn) submitBtn.disabled = true;

      try {
        const inputs = Array.from(performanceEditForm.querySelectorAll('input[data-field][data-index]'));
        const updatedWorkouts = latestWorkoutSession.workouts.map((exercise, index) => {
          const metrics = normalizeExercisePerformance(exercise);
          const updates = inputs.filter((input) => Number(input.dataset.index) === index);
          updates.forEach((input) => {
            const value = parseNumber(input.value);
            if (input.dataset.field === 'performedSets') metrics.performedSets = value;
            if (input.dataset.field === 'performedReps') metrics.performedReps = value;
            if (input.dataset.field === 'performedWeight') metrics.performedWeight = value;
          });
          return {
            ...exercise,
            targetSets: metrics.targetSets,
            targetReps: metrics.targetReps,
            targetDurationSec: metrics.targetDurationSec,
            performedSets: metrics.performedSets,
            performedReps: metrics.performedReps,
            performedWeight: metrics.performedWeight,
            watchedSeconds: metrics.watchedSeconds,
            percentWatched: metrics.percentWatched,
            videoDurationSec: metrics.videoDurationSec,
            videoCompleted: metrics.completed
          };
        });
        ...
        const res = await fetch('/api/workouts/session', {
          method: 'PATCH',
          headers: { 'Content-Type': 'application/json' },
          body: JSON.stringify({ sessionId: latestWorkoutSession.id, workouts: updatedWorkouts })
        });
        if (!res.ok) throw new Error('Failed to update performance details');
        const payload = await res.json();
        latestWorkoutSession = payload?.session || latestWorkoutSession;
        performanceEditModal?.classList.remove('show');
        renderPerformanceMilestones();
      } catch (err) {
        console.error('Failed to update performance details', err);
        showPromptAlert('Failed to update performance details. Please try again.');
      } finally {
        if (submitBtn) submitBtn.disabled = false;
      }
    });
