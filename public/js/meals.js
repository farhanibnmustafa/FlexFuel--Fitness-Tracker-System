document.addEventListener('DOMContentLoaded', async () => {
  ...
  const setPlanLoadingPlaceholders = () => {
    if (mealTableBody) {
    ...
    if (mealTableFoot) {
      ...
      [1, 2, 3, 4].forEach((idx) => {
        if (cells[idx]) cells[idx].textContent = '—';
      });
    }
    if (planCaloriesEl) {
      planCaloriesEl.textContent = '—';
    }
    if (macroSummaryEl) {
      macroSummaryEl.textContent = 'Loading meal plan...';
    }
    if (calorieList) {
    ...
    macroItems.forEach((item) => {
      ...
      if (consumedEl) consumedEl.textContent = '—';
  ...
  const PLAN_CACHE_KEY = 'flexfuel.meals.planCache';
  const PLAN_CACHE_MAX_AGE_MS = 6 * 60 * 60 * 1000;
  const LAST_USER_KEY = 'flexfuel.meals.lastUserKey';
  ...
  let mealUserKey = readLastUserKey();
  ...
  const syncMealCompletionsFromServer = async () => {
    if (!hasAuthenticatedSession || !mealUserKey || mealUserKey === 'anonymous') return;
    ...
    try {
      ...
      const state = loadStoredCompletions();
      ...
      localStorage.setItem(getMealCompletionStorageKey(), JSON.stringify(state));
      loadCompletionStateForCurrentUser();

      const missingEntries = Object.entries(state).flatMap(([date, ids]) =>
        (ids || [])
  ...
  const persistMealCompletionChange = async (dateKey, mealId, complete) => {
    if (!mealUserKey || mealUserKey === 'anonymous' || !dateKey || !mealId) return;
    try {
      await fetch('/api/meal-completions', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
          entries: [
            complete
              ? { meal_date: dateKey, meal_id: mealId }
              : { meal_date: dateKey, meal_id: mealId, remove: true }
          ]
        })
      });
    } catch (err) {
      console.warn('Failed to persist meal completion', err);
    }
  };
  ...
  function readLastUserKey() {
    try {
      return localStorage.getItem(LAST_USER_KEY) || 'anonymous';
    } catch (err) {
      return 'anonymous';
    }
  }

  function writeLastUserKey(value) {
    try {
      localStorage.setItem(LAST_USER_KEY, value || 'anonymous');
    } catch (err) {
      /* ignore */
    }
  }

  const loadPlanCache = (expectedUserKey) => {
    try {
      const raw = localStorage.getItem(PLAN_CACHE_KEY);
      const parsed = raw ? safeParseJSON(raw, null) : null;
      if (!parsed || !parsed.savedAt || !parsed.mealsByDate) return null;
      if (Date.now() - Number(parsed.savedAt) > PLAN_CACHE_MAX_AGE_MS) return null;
      if (expectedUserKey && parsed.userKey && parsed.userKey !== expectedUserKey) return null;
      return parsed;
    } catch (err) {
      console.warn('Failed to read plan cache', err);
      return null;
    }
  };

  const savePlanCache = (mealsByDate, source) => {
    if (!mealsByDate || typeof mealsByDate !== 'object') return;
    try {
      ...
      console.warn('Failed to persist plan cache', err);
    }
  };
  ...
  const getBaseMealFromPlan = (dateKey, mealId) =>
    mealPlanData[dateKey]?.meals?.find((meal) => meal.id === mealId) || null;
  ...
  const markMealRemoved = (dateKey, mealId) => {
    if (!dateKey || !mealId) return;
    if (!customMealsByDate[dateKey]) {
      customMealsByDate[dateKey] = {};
    }
    customMealsByDate[dateKey][mealId] = { removed: true };
  ...
  const setPlanCollections = (meals, options = {}) => {
    const { pruneOverrides = true } = options;
  ...
  setPlanCollections({}, { pruneOverrides: false });
  planLoading = true;
  applyActiveMealDateSelection();

  let usedCachedPlan = false;
  const cachedPlan = loadPlanCache(mealUserKey) || loadPlanCache();
  if (cachedPlan?.mealsByDate && Object.keys(cachedPlan.mealsByDate).length) {
    setPlanCollections(cachedPlan.mealsByDate, { pruneOverrides: false });
    planLoading = false;
    applyActiveMealDateSelection();
    queueMicrotask(() => {
      renderCalendar();
  ...
  const isMealDayComplete = (dateKey) => {
    if (!dateKey) return false;
    const planData = getPlanDataForDate(dateKey) || mealPlanData[dateKey] || null;
    const meals = Array.isArray(planData?.meals) ? planData.meals.filter((meal) => meal && !meal.removed) : [];
    if (!meals.length) return true;
    const completionSet = getCompletionSet(dateKey);
    return meals.every((meal) => completionSet.has(meal.id));
  };

  function getActiveDayStorageKey() {
    return `${ACTIVE_DAY_STORAGE_KEY}.${mealUserKey || 'anonymous'}`;
  }

  function readActiveDay() {
    try {
      const raw = localStorage.getItem(getActiveDayStorageKey());
  ...
  function writeActiveDay(iso) {
    if (!iso) return;
    try {
      localStorage.setItem(getActiveDayStorageKey(), iso);
    } catch (err) {
      /* ignore */
    }
  ...
  async function fetchActiveDayFromServer() {
    if (!hasAuthenticatedSession || !mealUserKey || mealUserKey === 'anonymous') return null;
    try {
  ...
  async function persistActiveDayToServer(iso) {
    if (!hasAuthenticatedSession || !mealUserKey || mealUserKey === 'anonymous' || !iso) return;
    try {
      ...
      console.warn('Failed to save active day', err);
    }
  }

  async function syncActiveDayFromServer() {
    const localIso = readActiveDay();
    const serverIso = await fetchActiveDayFromServer();
  ...
  const syncWorkoutCompletionsFromServer = async () => {
    ...
    try {
      ...
      const data = await res.json();
      const entries = Array.isArray(data?.entries) ? data.entries : [];
      const serverDates = new Set(entries.map((entry) => entry?.workout_date).filter(Boolean));
      const raw = localStorage.getItem('ff-workout-completion-state');
      const state = raw ? JSON.parse(raw) : {};
      const bucket = state[mealUserKey] || state.anonymous || {};
      ...
      state[mealUserKey] = bucket;
      localStorage.setItem('ff-workout-completion-state', JSON.stringify(state));
  ...
  const isWorkoutCompleteForDate = (dateIso) => {
    ...
    try {
      ...
      const bucket = parsed[mealUserKey] || parsed.anonymous || {};
  ...
  const isActivityDayComplete = (dateIso) => isMealDayComplete(dateIso) && isWorkoutCompleteForDate(dateIso);

  function resolveActiveMealDateKey() {
    const todayKey = isoFromDate(new Date());
    let activeKey = readActiveDay();
    if (activeKey && !mealPlanData[activeKey]) {
      activeKey = null;
    }
    if (!activeKey) {
      activeKey = mealPlanData[todayKey] ? todayKey : currentDateKey;
    }
    if (activeKey && todayKey && activeKey < todayKey) {
      let guard = 0;
      while (activeKey < todayKey && isActivityDayComplete(activeKey) && guard < 31) {
        const nextKey = shiftIsoDate(activeKey, 1);
        if (!nextKey || nextKey > todayKey) break;
        if (!mealPlanData[nextKey]) break;
        activeKey = nextKey;
        guard += 1;
      }
    }
    if (activeKey) writeActiveDay(activeKey);
    return activeKey || currentDateKey;
  }

  function applyActiveMealDateSelection() {
    const resolvedKey = resolveActiveMealDateKey();
    if (!resolvedKey) return;
    currentDateKey = resolvedKey;
    pendingDateKey = currentDateKey;
  ...
  const maybeAdvanceMealDay = () => {
    const now = new Date();
    const nextKey = resolveActiveMealDateKey();
    if (nextKey && nextKey !== currentDateKey) {
  ...
  const planExportPdfBtn = document.querySelector('.plan-export-pdf');
  ...
  const exportMealPlanCsv = (dateKey) => {
    const planData = getPlanDataForDate(dateKey) || mealPlanData[dateKey] || null;
    const meals = (planData?.meals || []).filter((meal) => !meal.removed);
    ...
    const rows = [
      ['Date', dateKey || ''],
      ['Plan', planData?.label || planData?.subtitle || ''],
      [],
      ['Meal Type', 'Suggested Foods', 'Quantity', 'Calories (kcal)', 'Protein (g)', 'Carbs (g)', 'Fat (g)'],
      ...meals.map((meal) => [
        meal.type || '',
        meal.foods || '',
        meal.quantity || '',
    ...
    link.href = url;
    link.download = `meal-plan-${dateKey || 'plan'}.csv`;
  ...
  const exportMealPlanPdf = (dateKey) => {
    const planData = getPlanDataForDate(dateKey) || mealPlanData[dateKey] || null;
    const meals = (planData?.meals || []).filter((meal) => !meal.removed);
    if (!meals.length) return;
    ...
    const title = escapeHtml(planData?.label || planData?.subtitle || 'Meal Plan');
    const dateLabel = escapeHtml(dateKey || '');
    const rowsHtml = meals
      .map(
        (meal) => `
        <tr>
          <td>${escapeHtml(meal.type || '')}</td>
          <td>${escapeHtml(meal.foods || '')}</td>
          <td>${escapeHtml(meal.quantity || '')}</td>
          <td>${escapeHtml(formatCalorieValue(meal.calories || 0))}</td>
          <td>${escapeHtml(formatMacroValue(meal.protein || 0))}</td>
          <td>${escapeHtml(formatMacroValue(meal.carbs || 0))}</td>
          <td>${escapeHtml(formatMacroValue(meal.fat || 0))}</td>
        </tr>`
      )
      .join('');
          ...
          <table>
            ...
            <tfoot>
              <tr>
                ...
                <td></td>
                <td></td>
                <td>${escapeHtml(formatCalorieValue(totals.calories))}</td>
                <td>${escapeHtml(formatMacroValue(totals.protein))}</td>
                <td>${escapeHtml(formatMacroValue(totals.carbs))}</td>
                <td>${escapeHtml(formatMacroValue(totals.fat))}</td>
              </tr>
            </tfoot>
          </table>
        </body>
  ...
  const makeFoodKey = (mealType, foodId) => `${normalizeMealType(mealType)}:${foodId}`;
  ...
  const findFoodMatches = (mealTypeKey, macroKey, originalMeal) => {
    const library = FOOD_LIBRARY[mealTypeKey]?.[macroKey] || [];
  ...
  const populateFoodSelect = () => {
    ...
    if (!mealModalState.mealTypeKey || !mealModalState.macro || !mealModalState.originalMeal) {
      foodSelect.disabled = true;
      return;
    }

    const matches = findFoodMatches(
      mealModalState.mealTypeKey,
      mealModalState.macro,
      mealModalState.originalMeal
    );
    if (!matches.length) {
    ...
    matches.forEach((food) => {
      ...
      option.dataset.mealType = mealModalState.mealTypeKey;
  ...
  const updateFavoriteToggleState = () => {
    ...
    const key =
      mealModalState.mealTypeKey && mealModalState.foodId
        ? makeFoodKey(mealModalState.mealTypeKey, mealModalState.foodId)
        : '';
    ...
    mealFavoriteToggle.dataset.mealType = mealModalState.mealTypeKey || '';
  ...
  const buildMealTypeOptions = () => {
    ...
    (plan?.meals || []).forEach((meal) => {
      const option = document.createElement('option');
      const mealTypeKey = normalizeMealType(meal.type || meal.label);
      ...
      option.dataset.mealType = mealTypeKey;
  ...
  const setMealTypeInModal = (mealId) => {
    if (!mealTypeSelect || !mealId) return;
    const optionExists = Array.from(mealTypeSelect.options).some((option) => option.value === mealId);
    if (!optionExists) return;
    mealTypeSelect.value = mealId;
    onMealTypeChange({ target: mealTypeSelect });
  };

  const openMealModal = (preferredMealId = '') => {
    if (!mealModalBackdrop || !mealModal || mealModalBackdrop.dataset.open === 'true') return;
    resetMealModal();
    buildMealTypeOptions();
    if (preferredMealId) {
      setMealTypeInModal(preferredMealId);
    }
  ...
  const applyFavoriteToModal = (favorite) => {
    ...
    const favoriteTypeKey = normalizeMealType(favorite.mealType);
    let matchingMeal = planMeals.find((meal) =>
      normalizeMealType(meal.type || meal.label) === favoriteTypeKey
    );
    if (!matchingMeal && favorite.mealSlotId) {
      matchingMeal = planMeals.find((meal) => meal.id === favorite.mealSlotId);
    }
    if (!matchingMeal && favorite.mealType) {
      matchingMeal = planMeals.find((meal) => meal.id === favorite.mealType);
    }
    if (!matchingMeal) {
      if (mealModalSubtitle) {
    ...
    const mealTypeKey = normalizeMealType(matchingMeal.type || favorite.mealType);
    setActiveModalTab('search');
    mealModalState.mealId = matchingMeal.id;
    mealModalState.mealTypeKey = mealTypeKey;
    mealModalState.originalMeal = getBaseMealFromPlan(currentDateKey, matchingMeal.id);
    mealModalState.macro = favorite.macro || 'protein';
    const selectedFood = { ...favorite, mealType: mealTypeKey, macro: favorite.macro || 'protein' };
    mealModalState.foodId = favorite.foodId;
    mealModalState.selectedFood = selectedFood;
    if (mealTypeSelect) {
      mealTypeSelect.value = matchingMeal.id;
    }
    if (macroSelect) {
      macroSelect.innerHTML = macroSelectInitialMarkup;
  ...
  const onMealTypeChange = (event) => {
    ...
    const fallbackTypeKey = normalizeMealType(getBaseMealFromPlan(currentDateKey, value)?.type || '');
    const mealTypeKey = normalizeMealType(selectedOption?.dataset.mealType || fallbackTypeKey);
    mealModalState.mealId = value;
    mealModalState.mealTypeKey = mealTypeKey;
    mealModalState.originalMeal = value ? getBaseMealFromPlan(currentDateKey, value) : null;
    mealModalState.macro = '';
    mealModalState.foodId = '';
    mealModalState.selectedFood = null;
    if (macroSelect) {
  ...
  const onFoodChange = (event) => {
    ...
    const mealType = option?.dataset.mealType || mealModalState.mealTypeKey;
  ...
  const toggleFavoriteSelection = () => {
    if (!mealModalState.mealId || !mealModalState.mealTypeKey || !mealModalState.foodId || !mealModalState.selectedFood) {
      return;
    }
    const key = makeFoodKey(mealModalState.mealTypeKey, mealModalState.foodId);
    if (favoriteFoods[key]) {
      delete favoriteFoods[key];
    } else {
      favoriteFoods[key] = {
        ...mealModalState.selectedFood,
        mealType: mealModalState.mealTypeKey,
        mealSlotId: mealModalState.mealId,
  ...
  const renderMealOptionSelect = (meal) => {
    if (!meal || meal.removed || !Array.isArray(meal.options) || meal.options.length < 2) return '';
  ...
  const attachMealListeners = (dateKey) => {
    ...
    mealItems.forEach((item) => {
      ...
      toggleBtn.addEventListener('click', () => {
        ...
        persistCompletions();
        persistMealCompletionChange(dateKey, mealId, currentlyComplete);
        updateAll();
  ...
  function renderMealPlan(dateKey) {
    ...
    const completionSet = getCompletionSet(dateKey);
    const isLoading = planLoading && !meals.length;
    ...
    const activeMeals = meals.filter((meal) => !meal.removed);
    ...
    const displayTargetCalories = totals.calories;
    if (planCaloriesEl) {
      planCaloriesEl.textContent = activeMeals.length
        ? `${formatCalorieValue(displayTargetCalories)} kcal`
        : '0 kcal';
    }
    if (macroSummaryEl) {
      macroSummaryEl.textContent = activeMeals.length
    ...
    applyTargetCalories(displayTargetCalories);
    ...
    if (mealTableBody) {
      if (isLoading) {
        ...
        mealTableBody.innerHTML = meals
          .map(
            (meal) => {
              const isRemoved = Boolean(meal.removed);
              const foodsText = isRemoved ? 'Removed — use Log Meal to replace' : meal.foods;
              const quantityText = isRemoved ? '—' : meal.quantity;
              const caloriesValue = isRemoved ? 0 : meal.calories;
              const proteinValue = isRemoved ? 0 : meal.protein;
              const carbsValue = isRemoved ? 0 : meal.carbs;
              const fatValue = isRemoved ? 0 : meal.fat;
              const optionSelect = renderMealOptionSelect(meal);
              const actionAttr = isRemoved ? 'data-meal-restore' : 'data-meal-remove';
              const actionLabel = isRemoved
                ? `Restore ${meal.type.toLowerCase()} meal`
                : `Remove ${meal.type.toLowerCase()} meal`;
              return `<tr>
              <td>${meal.type}</td>
              <td>
                <div class="meal-foods-cell">
                  <span class="meal-foods-text">${foodsText}</span>
                  ${optionSelect}
                </div>
              </td>
              <td>${quantityText}</td>
              <td>${formatCalorieValue(caloriesValue)}</td>
              <td>${formatMacroValue(proteinValue)}</td>
              <td>${formatMacroValue(carbsValue)}</td>
              <td>${formatMacroValue(fatValue)}</td>
              <td><button class="table-action" type="button" ${actionAttr} data-meal-id="${meal.id}" aria-label="${actionLabel}"></button></td>
            </tr>`;
            }
          )
          .join('');
    ...
    if (calorieList) {
      if (isLoading) {
        calorieList.innerHTML = '';
      } else if (activeMeals.length) {
        calorieList.innerHTML = activeMeals
          .map((meal) => {
    ...
    if (macroTrendEmpty) {
      macroTrendEmpty.textContent = isLoading
        ? ''
        : activeMeals.length
  ...
  const loadDynamicPlan = async () => {
    ...
    try {
      ...
      profile = payload?.user?.profile ? { ...payload.user.profile } : {};
      userId = payload?.user?.id || null;
      hasAuthenticatedSession = Boolean(userId);
      goalNutritionTarget = readGoalNutritionTarget(userId);
      if (window.FFStreak) {
        const initSummary = window.FFStreak.init(userId);
        if (initSummary?.userKey) {
          mealUserKey = initSummary.userKey;
          writeLastUserKey(mealUserKey);
          loadCompletionStateForCurrentUser();
          applyActiveMealDateSelection();
          void Promise.allSettled([
            syncActiveDayFromServer(),
      ...
      console.warn('Unable to load profile for meal plan', err);
    }
      ...
      try {
        ...
        if (planResponse.ok) {
          ...
          if (planPayload?.mealsByDate && Object.keys(planPayload.mealsByDate).length) {
            savePlanCache(planPayload.mealsByDate, planPayload?.source || 'supabase');
            if (!usedCachedPlan) {
    ...
    try {
      ...
      if (planResult?.mealsByDate && Object.keys(planResult.mealsByDate).length) {
        savePlanCache(planResult.mealsByDate, 'flexplan');
        if (!usedCachedPlan) {
  ...
  mealTableBody?.addEventListener('click', (event) => {
    const removeBtn = event.target?.closest?.('[data-meal-remove]');
    if (removeBtn) {
      const mealId = removeBtn.dataset.mealId;
      markMealRemoved(currentDateKey, mealId);
      renderMealPlan(currentDateKey);
      return;
    }
    const restoreBtn = event.target?.closest?.('[data-meal-restore]');
    if (restoreBtn) {
      const mealId = restoreBtn.dataset.mealId;
      removeMealOverride(currentDateKey, mealId);
      renderMealPlan(currentDateKey);
    }
  });
  ...
  logMealBtn?.addEventListener('click', () => {
    ...
    const plan = getPlanDataForDate(currentDateKey);
    const removedMeal = plan?.meals?.find((meal) => meal.removed);
    openMealModal(removedMeal?.id || '');
  });

  planExportPdfBtn?.addEventListener('click', () => {
    exportMealPlanPdf(currentDateKey);
  });
