...
export function clearExerciseVideoLibraryCache(schemaVersion) {
  if (!schemaVersion) {
    videoLibraryCache.clear();
    return;
  }
  const versionKey =
    typeof schemaVersion === 'string' && schemaVersion.trim()
      ? schemaVersion.trim()
      : 'exercise_videos_v1';
  videoLibraryCache.delete(versionKey);
}
...
function extractYouTubeId(url) {
  ...
  const embedMatch = url.match(/youtube(?:-nocookie)?\.com\/embed\/([a-zA-Z0-9_-]+)/i);
  ...
  const legacyMatch = url.match(/youtube\.com\/v\/([a-zA-Z0-9_-]+)/i);
...
function buildExerciseVideoLibrary(config, schemaVersion) {
  ...
  config.categories.forEach((category, categoryIndex) => {
    ...
    workouts.forEach((workout, workoutIndex) => {
      ...
      const rawThumbnail = typeof workout.thumbnail === 'string' ? workout.thumbnail.trim() : '';
      const thumbnail = rawThumbnail || (videoId ? `https://i.ytimg.com/vi/${videoId}/hqdefault.jpg` : null);
